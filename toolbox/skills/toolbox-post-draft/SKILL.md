...
9. Run `scripts/serve-local.mjs` and preview `toolbox/preview-post.html?tool=<tool>&contentOnly=1`. Verify the tool, post, label search, search results, archive, preserved Blogger result list, and compact footer at mobile and desktop sizes, including input, error, long content, overflow, iframe height, hashtags, empty and matching search states, fuzzy keyword results, FAQ, cumulative patch notes, floating return-to-search button, and absence of mobile auto-focus. Record QA in the new `patch-notes.json`.
