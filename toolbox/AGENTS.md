...
- 자동 작업의 선택부터 공개 검증·결과 확인까지 상위 프로젝트의 `AUTOMATED-WORKFLOW.md`를 단일 절차로 따른다.
...
- 비교할 두 문장처럼 장문 텍스트 입력 영역이 둘 이상이면 데스크톱에서도 좌우로 나누지 않고 위/아래 한 열로 고정한다. 각 영역은 전체 폭과 충분한 높이를 확보하고 모바일 세로 화면에서도 같은 순서를 유지한다.
