...
# 기능
- 생성 결과로 바로 이동하는 버튼과 Alt+G, Alt+R 초기화
- `Alt+L` 언어 전환·`lang` 직접 열기와 `Ctrl/⌘+Shift+S` 생성 이미지 저장
...
## 2026-08-28 파일 입출력 동선

- QR·바코드 원문은 1MB 이하 텍스트 파일에서 불러오거나 UTF-8 TXT로 저장할 수 있다.
- 결과 저장은 `Ctrl/⌘ + Enter`로도 실행할 수 있다.

## 2026-09-03 입력·예제 단축키

- `Alt+U`로 내용 입력에 돌아가고 `Alt+S`로 Wi-Fi QR 예제를 적용한다.
