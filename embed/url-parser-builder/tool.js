...
const clearQuery=document.createElement('button');clearQuery.className='st-secondary';clearQuery.id='url-clear-query';clearQuery.type='button';clearQuery.dataset.ko='쿼리 전체 비우기';clearQuery.dataset.en='Clear all query parameters';clearQuery.textContent=tr('쿼리 전체 비우기','Clear all query parameters');removeTracking.after(clearQuery);clearQuery.addEventListener('click',()=>{if(!parsedUrl)return setStatus('먼저 URL을 분석해 주세요.','Parse a URL first.',true);const count=$('#url-query-list').children.length;$('#url-query-list').replaceChildren();rebuild();setStatus(count?`${count}개의 쿼리 항목을 모두 비웠습니다.`:'비울 쿼리 항목이 없습니다.',count?`Cleared all ${count} query parameter${count===1?'':'s'}.`:'There are no query parameters to clear.');});
