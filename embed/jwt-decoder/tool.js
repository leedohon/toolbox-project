...
const copyTimes=document.createElement('button');copyTimes.className='st-secondary';copyTimes.id='jwt-copy-times';copyTimes.type='button';copyTimes.dataset.ko='시간 요약 복사';copyTimes.dataset.en='Copy time summary';copyTimes.textContent=tr('시간 요약 복사','Copy time summary');$('#jwt-copy-summary').after(copyTimes);copyTimes.addEventListener('click',async()=>{const value=$('#jwt-times').textContent.trim();if(!payloadText||!value)return status('먼저 JWT 내용을 여세요.','Decode a JWT first.',true);if(await copyText(value)){$('#jwt-fallback').hidden=true;status('시간 클레임 요약을 복사했습니다.','Time claim summary copied.');}else{$('#jwt-fallback').value=value;$('#jwt-fallback').hidden=false;status('직접 복사할 시간 요약을 표시했습니다.','The time summary is shown for manual copying.');}});
