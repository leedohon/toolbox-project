...
if (fromRunIndex >= 0) {
  ...
  if (!runId) throw new Error('Usage: node scripts/apply-public-indexing-policy.mjs --from-run RUN_ID');
...
if (!selectedTools && nextRootHtml !== rootHtml) {
  await fs.writeFile(rootIndex, nextRootHtml);
  updated += 1;
}
if (!selectedTools && rootCanonical) canonicalCount += 1;

console.log(`Applied noindex to ${selectedTools ? selectedTools.size : manifests.size + 1} public execution page(s); ${canonicalCount} canonical target(s); ${updated} file(s) changed.`);
