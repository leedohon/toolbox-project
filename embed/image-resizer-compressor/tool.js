...
$('#img-scale').addEventListener('change',()=>{if(!source||$('#img-scale').value==='custom')return;const scale=Number($('#img-scale').value)/100;$('#img-width').value=Math.max(1,Math.round(source.width*scale));$('#img-height').value=Math.max(1,Math.round(source.height*scale));invalidateOutput();});
