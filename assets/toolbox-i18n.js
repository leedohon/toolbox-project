(function () {
  ...
  const isHubChild = query.get('toolbox-hub-child') === '1';
  ...
  const exact = new Map(Object.entries({
    ...
    '색상 변환·대비 검사기': 'Color converter and contrast checker', '글자색': 'Text color', '배경색': 'Background color', '글자색 선택': 'Choose text color', '배경색 선택': 'Choose background color', '색상 서로 바꾸기': 'Swap colors', '변환값 복사': 'Copy color values', '색상을 바꾸면 대비와 변환값이 즉시 갱신됩니다.': 'Change a color to update contrast and converted values instantly.', '읽기 쉬운 색상인가요?': 'Are these colors readable?', '대비 검사 결과': 'Contrast check result',
    'Unix 시간 변환기': 'Unix timestamp converter', 'Unix 시간 → 날짜': 'Unix time → date', 'Unix 시간': 'Unix time', '숫자 단위': 'Number unit', '자동 판별': 'Auto detect', '초': 'Seconds', '밀리초': 'Milliseconds', '날짜로 변환': 'Convert to date', '현재 시간 넣기': 'Use current time', '변환된 날짜': 'Converted date', '날짜 → Unix 시간': 'Date → Unix time', '내 지역 날짜와 시간': 'Local date and time', 'Unix 시간으로 변환': 'Convert to Unix time', '초·밀리초를 자동 판별하며 지역 시간과 UTC를 함께 표시합니다.': 'Auto-detect seconds or milliseconds and show local time with UTC.', 'Unix 시간 결과': 'Unix time result',
    '글자수·텍스트 정리기': 'Text counter and cleaner', '글자수를 세거나 정리할 텍스트를 입력하세요.': 'Enter text to count or clean.', '공백 포함': 'With spaces', '공백 제외': 'Without spaces', '단어': 'Words', '줄': 'Lines', '문장': 'Sentences', 'UTF-8 용량': 'UTF-8 size', '텍스트 정리 옵션': 'Text cleanup options', '줄 앞뒤 공백 제거': 'Trim each line', '연속 공백 하나로': 'Collapse repeated spaces', '빈 줄 하나만 남기기': 'Keep one blank line', '줄바꿈을 공백으로': 'Replace line breaks with spaces', '선택한 방식으로 정리': 'Clean with selected options', '입력과 동시에 글자수와 UTF-8 용량을 계산합니다.': 'Count characters and UTF-8 size while you type.', '정리 결과': 'Cleaned result', '정리한 결과가 여기에 표시됩니다.': 'The cleaned result appears here.',
    ...
    'ASCII 10진수 코드': 'ASCII decimal codes', 'Unicode 코드 포인트': 'Unicode code points', 'UTF-8 16진수': 'UTF-8 hexadecimal', 'UTF-8 2진수': 'UTF-8 binary',
    '원문과 코드를 서로 변환할 형식을 선택하세요.': 'Choose a format to convert between text and codes.', '변환할 문장을 입력하세요.': 'Enter text to convert.',
    'ASCII 인코딩': 'ASCII encode', 'ASCII 디코딩': 'ASCII decode', 'ASCII 입력': 'ASCII input', 'ASCII 결과': 'ASCII result',
    'Unicode 인코딩': 'Unicode encode', 'Unicode 디코딩': 'Unicode decode', 'Unicode 입력': 'Unicode input', 'Unicode 결과': 'Unicode result',
    'UTF-8 16진수 인코딩': 'UTF-8 hexadecimal encode', 'UTF-8 16진수 디코딩': 'UTF-8 hexadecimal decode', 'UTF-8 16진수 입력': 'UTF-8 hexadecimal input', 'UTF-8 16진수 결과': 'UTF-8 hexadecimal result',
    'UTF-8 2진수 인코딩': 'UTF-8 binary encode', 'UTF-8 2진수 디코딩': 'UTF-8 binary decode', 'UTF-8 2진수 입력': 'UTF-8 binary input', 'UTF-8 2진수 결과': 'UTF-8 binary result',
    'ASCII 변환을 완료했습니다.': 'ASCII conversion complete.', 'Unicode 변환을 완료했습니다.': 'Unicode conversion complete.', 'UTF-8 16진수 변환을 완료했습니다.': 'UTF-8 hexadecimal conversion complete.', 'UTF-8 2진수 변환을 완료했습니다.': 'UTF-8 binary conversion complete.',
    'ASCII 변환 방식을 선택했습니다.': 'ASCII conversion selected.', 'Unicode 변환 방식을 선택했습니다.': 'Unicode conversion selected.', 'UTF-8 16진수 변환 방식을 선택했습니다.': 'UTF-8 hexadecimal conversion selected.', 'UTF-8 2진수 변환 방식을 선택했습니다.': 'UTF-8 binary conversion selected.',
    'ASCII는 영문, 숫자, 기본 기호만 지원합니다. 한글과 이모지는 Unicode를 선택하세요.': 'ASCII supports only English letters, numbers, and basic symbols. Choose Unicode for Korean text and emoji.',
    'ASCII 코드는 0부터 127까지의 숫자로 입력해 주세요.': 'Enter ASCII codes as numbers from 0 to 127.', 'Unicode 코드는 U+AC00처럼 입력해 주세요.': 'Enter Unicode codes such as U+AC00.',
    '16진수는 두 자리 바이트 형식으로 입력해 주세요.': 'Enter hexadecimal as two-digit bytes.', '2진수는 8자리 바이트 형식으로 입력해 주세요.': 'Enter binary as eight-bit bytes.',
    '예: 72 101 108 108 111': 'e.g. 72 101 108 108 111', '예: U+D55C U+AE00': 'e.g. U+0048 U+0069', '예: ED 95 9C EA B8 80': 'e.g. 48 69', '예: 01001000 01101001': 'e.g. 01001000 01101001',
    ...
    '결과 공유': 'Share result', '결과 코드를 보내면 받은 사람이 같은 결과를 열 수 있습니다.': 'Send the result code so the recipient can open the same result.', '결과 코드를 보내면 받은 사람이 같은 숫자를 열 수 있습니다.': 'Send the result code so the recipient can open the same numbers.', '결과 링크를 보내면 받은 사람이 블로그에서 같은 사다리 결과를 바로 볼 수 있습니다.': 'Send a result link so the recipient can immediately see the same ladder result on the blog.', '결과 링크를 보내면 받은 사람이 블로그에서 같은 당첨 결과를 바로 볼 수 있습니다.': 'Send a result link so the recipient can immediately see the same winning result on the blog.', '결과 링크를 보내면 받은 사람이 블로그에서 같은 숫자를 바로 볼 수 있습니다.': 'Send a result link so the recipient can immediately see the same numbers on the blog.', '결과 링크를 보내면 받은 사람이 블로그에서 같은 팀을 바로 볼 수 있습니다.': 'Send a result link so the recipient can immediately see the same teams on the blog.', '받은 결과 코드': 'Received result code', '친구에게 받은 코드를 붙여넣으세요.': 'Paste the result code you received.',
    '결과 링크 복사': 'Copy result link', '결과 코드 복사': 'Copy result code', '같은 결과 열기': 'Open same result', 'PNG 저장': 'Save PNG',
    ...
    '결과를 만든 뒤 코드 복사나 PNG 저장을 사용할 수 있습니다.': 'Create a result before copying a code or saving a PNG.', '결과를 만든 뒤 링크·코드 복사나 PNG 저장을 사용할 수 있습니다.': 'Create a result before copying a link or code, or saving a PNG.',
    ...
    '같은 결과를 열었습니다.': 'Opened the same result.', '공유 링크의 같은 결과를 열었습니다.': 'Opened the same result from the shared link.', '공유 링크에서 같은 결과를 복원했습니다.': 'Restored the same result from the shared link.', '같은 결과가 바로 열리는 링크를 복사했습니다.': 'Copied a link that opens the same result.', '아래 결과 링크를 직접 복사해 주세요.': 'Copy the result link shown below manually.', '결과 링크가 메신저에서 보내기에는 너무 깁니다. 결과 코드나 PNG 저장을 이용해 주세요.': 'The result link is too long for a messenger. Use the result code or save a PNG.', '받은 결과 코드를 붙여넣어 주세요.': 'Paste the result code you received.',
  ...
  let language = ['ko', 'en'].includes(requestedLanguage)
    ? requestedLanguage
    : (localStorage.getItem(STORAGE_KEY) === 'en' ? 'en' : 'ko');
  let applying = false;

  if (['ko', 'en'].includes(requestedLanguage)) localStorage.setItem(STORAGE_KEY, language);
  ...
  function createSelector() {
    ...
    bar.setAttribute('aria-keyshortcuts', 'Alt+L');
  ...
  function start() {
    ...
    apply(document.body);
    if (!isHubChild) addEventListener('keydown', (event) => {
      if (event.key.toLowerCase() !== 'l' || !event.altKey || event.ctrlKey || event.metaKey || event.shiftKey || event.isComposing) return;
