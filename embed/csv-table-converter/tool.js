...
$('#csv-copy-headers').addEventListener('click',async()=>{if(!rows.length)return setStatus('복사할 열 이름이 없습니다.','There are no column names to copy.',true);const width=rows[0].length,headers=$('#csv-header').checked?rows[0]:Array.from({length:width},(_,index)=>tr(`열 ${index+1}`,`Column ${index+1}`)),value=headers.join('\t');if(await copyText(value)){setStatus('열 이름을 복사했습니다.','Column names copied.');$('#csv-fallback').hidden=true;}else{$('#csv-fallback').value=value;$('#csv-fallback').hidden=false;setStatus('직접 복사할 열 이름을 표시했습니다.','Column names are shown for manual copying.',true);}});
