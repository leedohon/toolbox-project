...
function syncDynamicLabels(){document.querySelectorAll('[data-ko][data-en]').forEach(element=>{element.textContent=tr(element.dataset.ko,element.dataset.en);});}
async function readClipboardImage(){try{const items=await navigator.clipboard.read();for(const item of items){const type=item.types.find(value=>value.startsWith('image/'));if(type){const blob=await item.getType(type);await load(new File([blob],'clipboard-image.'+(type==='image/jpeg'?'jpg':type.split('/')[1]),{type}));return;}}status(tr('클립보드에 PNG, JPEG 또는 WebP 이미지가 없습니다.','The clipboard has no PNG, JPEG, or WebP image.'),true);}catch{status(tr('클립보드 읽기가 차단되었습니다. 이미지를 복사한 뒤 Ctrl+V로 붙여 넣으세요.','Clipboard access was blocked. Copy an image and press Ctrl+V.'),true);}}
...
async function load(file){
  ...
  source=image;ratio=image.width/image.height;originalSize=file.size;$('#img-filename').value=sourceName;
...
$('#img-paste').addEventListener('click',readClipboardImage);
addEventListener('paste',event=>{const file=[...event.clipboardData.files].find(item=>item.type.startsWith('image/'));if(file)load(file);});
...
const socialPreset=document.createElement('button');socialPreset.className='st-secondary';socialPreset.id='img-social-preset';socialPreset.type='button';socialPreset.dataset.ko='공유용 1080px';socialPreset.dataset.en='Social 1080px';socialPreset.textContent=tr('공유용 1080px','Social 1080px');emailPreset.after(socialPreset);
...
socialPreset.addEventListener('click',()=>{if(!source)return status(tr('먼저 이미지를 선택해 주세요.','Choose an image first.'),true);const scale=Math.min(1,1080/Math.max(source.width,source.height));$('#img-scale').value='custom';$('#img-width').value=Math.max(1,Math.round(source.width*scale));$('#img-height').value=Math.max(1,Math.round(source.height*scale));$('#img-type').value='image/webp';$('#img-quality').value='85';$('#img-quality-value').textContent='85%';$('#img-quality-wrap').hidden=false;invalidateOutput(false);status(tr('긴 변 1080px·WebP 85% 공유 설정을 적용했습니다.','Applied 1080px long edge, WebP, and 85% quality for sharing.'));});
$('#img-download').addEventListener('click',()=>{if(!outputBlob)return;const ext={'image/jpeg':'jpg','image/webp':'webp','image/png':'png'}[outputBlob.type]||'png',link=document.createElement('a');const requested=$('#img-filename').value.replace(/\.[^.]+$/,'').replace(/[<>:"/\\|?*\u0000-\u001f]/g,'-').trim().slice(0,120);link.href=outputUrl;link.download=`${requested||sourceName}.${ext}`;link.click();status(tr('변환 이미지를 저장했습니다.','Saved the converted image.'));});
$('#img-reset').addEventListener('click',()=>{cleanup();source=null;sourceName='toolbox-image';$('#img-file').value='';$('#img-filename').value='toolbox-image';$('#img-preview-image').removeAttribute('src');$('#img-settings').hidden=true;$('#img-preview').hidden=true;$('#img-stats').hidden=true;status(tr('초기화했습니다. 새 이미지를 선택하세요.','Reset. Choose a new image.'));});
addEventListener('toolbox-language-change',syncDynamicLabels);
