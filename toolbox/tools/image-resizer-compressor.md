...
## 기능
- 변환 미리보기로 바로 이동하는 버튼과 Alt+G, Alt+R 초기화
- `Alt+L` 언어 전환·`lang` 직접 열기와 `Ctrl/⌘+Shift+S` 변환 이미지 저장
...
- `Alt+U`로 이미지 선택에 돌아가고 `Alt+S`로 샘플 이미지를 불러온다.

## 2026-09-04 단축키 수정

- 이미지가 없는 초기 상태에서도 `Alt+S`가 대표 샘플을 즉시 불러와 설정 화면을 연다.
