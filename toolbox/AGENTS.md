...
- 자동 작업의 트리거, 세부 워크플로, 결과 JSON과 확인 처리 기준은 상위 프로젝트의 `AUTOMATED-WORKFLOW.md`를 따른다.
- 자동 작업도 일반 릴리스와 동일하게 요구사항 기록, 버전 보존, 게시글 생성·수정, Blogger 반영, Git 커밋·푸시와 공개 배포 확인을 완료한다.
- Skill·Markdown 최적화의 버전, 측정 기준과 개선 대기열은 `toolbox/automation/knowledge-optimization.json`에서만 관리한다. 현재 작업과 경로가 겹치는 대기 항목만 최대 1건 처리하고 나머지는 보관한다.
...
- 앞으로 새로 작성하거나 수정하는 Blogger 게시글은 `toolbox/seo-rules.json`을 단일 원본으로 사용한다.
- 게시글마다 하나의 주 검색 의도를 정하고 제목, 리드, 사용 방법, 상세 설명, FAQ, 해시태그와 검색 키워드가 같은 의도를 자연스럽게 설명하게 한다.
- 정확한 도구명, 입력, 결과와 실제 사용 이점을 먼저 제시하고 H1은 한 번만, H2는 논리적인 사용자 흐름으로 구성한다.
- 키워드 반복·숨김 텍스트·중복 문구·관련 없는 분량 늘리기·운영자용 FAQ는 금지한다.
- SEO 적용 범위는 Blogger 게시글이며 실행 iframe, 기능 UI와 도구 로직에는 검색 유입용 문구를 추가하지 않는다.
...
- 메인 화면은 검색어가 없어도 도구 카드를 3열로 표시한다. 게시글·검색·라벨·보관함 등 내부 화면만 빈 검색 결과를 숨기고, 입력 뒤에 `outputs/tools.json`의 제목·설명·해시태그·20~30개 키워드에 일치하거나 유사한 결과를 가벼운 링크 목록으로 표시한다.
...
- 공개 게시글 제목은 기본적으로 `[초간단 툴박스] 도구명` 형식을 사용한다. 별도 카테고리가 확정된 도구는 `versions.json`의 `category`를 단일 원본으로 사용해 `[카테고리] 도구명`을 생성한다.
...
- 기존 전체 프로젝트를 한꺼번에 재작성하지 않는다. 새로 만드는 도구와 새로 통합·분리하는 도구부터 아래 구조를 순차 적용한다.
- 사용자가 방문하는 공개 진입점은 `embed/<통합도구>/index.html` 하나로 유지하고, 세부 기능은 독립 모듈 또는 기존 호환 엔진으로 분리한다. 공개 진입점은 모듈 선택, 높이 전달, 언어 상태와 오류 안내만 조정한다.
- 기능 모듈은 게시글 제목, 사이트 탐색, FAQ와 패치노트를 포함하지 않는다. 공통 게시글 템플릿과 기능 구현을 분리해 어느 한쪽을 바꿔도 다른 쪽을 전부 수정하지 않게 한다.
- 통합 대상의 기존 `index`, 버전 폴더, 게시글 URL과 공유 결과 코드는 삭제하거나 재배정하지 않는다. `versions.json`에 `status: retired`, `replacementTool`을 기록해 카탈로그에서만 제외하고 기존 주소는 새 통합 도구로 안내한다.
- 새 통합 도구는 새로운 고유 `index`를 배정한다. 은퇴 도구의 index는 재사용하지 않는다.
- 카테고리·게시글 제목 접두어는 `versions.json`의 `category`, 검색 명칭은 `title`, 기능 경로는 `tool`로 분리해 명칭 변경이 URL과 공유 코드에 영향을 주지 않게 한다.
- 통합 작업에서는 기존 결과 코드를 새 형식으로 일괄 변환하지 않는다. 기존 엔진을 호환 모듈로 유지하거나 명확한 이전 도구 안내를 제공해 이미 공유된 코드가 계속 열리게 한다.
