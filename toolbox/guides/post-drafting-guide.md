...
- 게시 제목은 `[초간단 툴박스] 도구명` 형식을 유지하고, 메인과 게시글 상단에서 같은 Openworld Blog 소개·도구 검색 UI를 사용한다.
- 검색어가 비어 있으면 링크를 숨기고, 입력 뒤에만 JSON 카탈로그의 제목·설명·해시태그·20~30개 키워드에 일치하거나 유사한 도구 링크를 표시한다.
